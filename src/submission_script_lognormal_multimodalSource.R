...
program <- function(mix_rna = NULL, ref_bulkRNA = NULL, 
  ...
  top_features<-readRDS("attachement/top_chosen_features_v4.rds")
  #top_chosen_features
  
  log_normal_regression <- function(b, A) {
    # Ensure all data is positive
    b <- log(b + 1e-6)  # Log-transform the mixture (add small constant for stability)
    A <- log(A + 1e-6)  # Log-transform the reference (add small constant for stability)
    
    # Solve using linear regression
    fit <- lm(b ~ A - 1)  # Use linear regression without intercept
    tmp_prop <- coef(fit) # Extract coefficients
    
    # Ensure proportions are non-negative and sum to 1
  ...
  if (!is.null(x = mix_rna)) {
    ...
    mix_rna <- mix_rna[top_features$genes, ]
    ref_bulkRNA <- ref_bulkRNA[top_features$genes, ]
    
    prop_rna <- apply(mix_rna, 2, function(b, A) {
      log_normal_regression(b = b, A = A)
    }, A = ref_bulkRNA)
    rownames(prop_rna) <- colnames(ref_bulkRNA)
  }
  
  ## Methylation Proportion Estimation
  if (!is.null(mix_met)) {
    #idx_feat <- intersect(rownames(mix_met), rownames(ref_met))
    #mix_met <- mix_met[idx_feat, ]
    #ref_met <- ref_met[idx_feat, ]
    
    #mix_met <- mix_met[top_features$probes, ]
    #ref_met <- ref_met[top_features$probes, ]
    # Filter probes that exist in mix_met and ref_met
	valid_probes <- intersect(top_features$probes, rownames(mix_met))
	valid_probes <- intersect(valid_probes, rownames(ref_met))  # Ensure overlap with both datasets

	# Subset mix_met and ref_met with valid probes
	mix_met <- mix_met[valid_probes, ]
	ref_met <- ref_met[valid_probes, ]
